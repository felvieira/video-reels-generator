...
impl VideoProcessor {
    ...
    pub async fn process_video(
        ...
        let output = Command::new(&self.ffmpeg_path)
            ...
            .arg(face_path.to_str().unwrap())
            .arg("-filter_complex")
            .arg("[0:v]scale=1080:1440:force_original_aspect_ratio=decrease[content];[1:v]scale=1080:480:force_original_aspect_ratio=increase[face];[content][face]vstack=shortest=1:inputs=2[combined];[combined]pad=1080:1920:(ow-iw)/2:(oh-ih)/2")
            .arg("-c:v")
            .arg("libx264")
            .arg("-preset")
            .arg(&quality_settings.preset)
